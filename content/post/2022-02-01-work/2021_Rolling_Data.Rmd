---
title: "NFL Weekly Updating Data"
output:
  html_document:
    theme: journal
    toc: yes
...
x_cpoe %>%
  ...
  labs(x = "Completion % Above Expected ",
       y = "EPA per play",
       title = "Graph 1: Quarterback Efficiency 2021",
...
sack_data %>%
  ...
  labs(x = "Total EPA Lost via Sacks",
       ...
       title = "Graph 2: EPA Lost by Sacks",
  ...
  geom_label(aes(x = -25, y = -2.5, label = "Fewer but Costly Sacks"), size = 2.5) +
  geom_label(aes(x = -70, y = -2.5, label = "Lots of Bad Sacks"),  color = "red", size = 2.5) +
  geom_label(aes(x = -70, y = -1.4, label = "Lots of Sacks"),  size = 2.5) +
  geom_label(aes(x = -22, y = -1.4, label = "Not Costly"), color = "dark green", size = 2.5) 
...
time_to_throw_vs_Sticks %>%
  ...
  labs(x = "Average Time to Throw",
       ...
       title = "Graph 3: QB Agressiveness",
  ...
  geom_label(aes(x = 3.05, y = -2.5, label = "Hold Long, Shallow"), color = "red", size = 2.5) +
  geom_label(aes(x = 2.42, y = -2.5, label = "Cowards"), size = 2.5) +
  geom_label(aes(x = 2.42, y = .8, label = "Quick, Deep"), color = "dark green", size = 2.5) +
  geom_label(aes(x = 3.05, y = .8, label = "Hero Ballers"), size = 2.5) +
  ylim(-2.75, 1.15) +
  xlim(2.35, 3.1)
```
...
seperation %>%
  ...
  #horizontal line with mean throw time
  geom_hline(yintercept = mean(seperation$avg_yac_above_expectation), color = "red", linetype = "dashed", alpha=0.5) +
  #vertical line with mean CPOE
  geom_vline(xintercept =  mean(seperation$avg_separation), color = "red", linetype = "dashed", alpha=0.5) +
  #add points for the QBs with the right colors
  #cex controls point size and alpha the transparency (alpha = 1 is normal)
  geom_point(color = seperation$team_color, cex=seperation$targets / 30, alpha = .6) +
  #add names using ggrepel, which tries to make them not overlap
  geom_text_repel(aes(label=player_short_name), size = 2.75 ) +
  #add a smooth line fitting cpoe + epa
  stat_smooth(geom='line', alpha=0.5, se=FALSE, method='lm')+
  #titles and caption
  labs(x = "Avg Seperation",
       y = "Average YAC Over Expected",
       title = "Graph 3: Individual WR Performance",
...
seperation %>%
  ...
  #horizontal line with mean throw time
  geom_hline(yintercept = mean(seperation$avg_separation), color = "red", linetype = "dashed", alpha=0.5) +
  #vertical line with mean CPOE
  geom_vline(xintercept =  mean(seperation$avg_cushion), color = "red", linetype = "dashed", alpha=0.5) +
  #add points for the QBs with the right colors
  #cex controls point size and alpha the transparency (alpha = 1 is normal)
  geom_point(color = seperation$team_color, cex=seperation$targets / 25, alpha = .6) +
  #add names using ggrepel, which tries to make them not overlap
  geom_text_repel(aes(label=player_short_name), size = 2.75 ) +
  #add a smooth line fitting cpoe + epa
  stat_smooth(geom='line', alpha=0.5, se=FALSE, method='lm')+
  #titles and caption
  labs(x = "Avg Cushion",
       y = "Average Seperation",
       title = "Graph 4: WR Seperation",
       caption = "Min 100 Targets") +
  #uses the black and white ggplot theme
  theme_bw() +
  #center title with hjust = 0.5
...
## 6. Team Receiver Data

```{r Team Sep Data, include=FALSE, message=FALSE}
seperation2 <- WR_nxt %>%
  filter(week>0) %>%
  group_by(team_abbr) %>%
  summarize(
...
seperation2 <- seperation2 %>% left_join(nfl_logos_df, by = c("team_abbr" = "team_code"))
```

```{r Team Seperation, echo=FALSE, message = FALSE, warning=FALSE}
seperation2 %>%
  ggplot(aes(x = mean_cush, y = mean_sep)) +
  #horizontal line with mean throw time
  geom_hline(yintercept = mean(seperation2$mean_sep), color = "red", linetype = "dashed", alpha=0.5) +
  #vertical line with mean CPOE
  geom_vline(xintercept =  mean(seperation2$mean_cush), color = "red", linetype = "dashed", alpha=0.5) +
  #add points for the QBs with the right colors
  #cex controls point size and alpha the transparency (alpha = 1 is normal)
  ...
  labs(x = "Avg Cushion",
       y = "Average Seperation",
       title = "Graph 5: Team WR Seperation") +
  #uses the black and white ggplot theme
  theme_bw() +
  #center title with hjust = 0.5
  theme(
...
stargazer(EPA_by_Run_Gap_2020,
          ...
          style = "aer",
          type = "html",
          title = "Table 1: 2020 EPA by Run Gap")
```
...
stargazer(EPA_by_Run_Gap,
          ...
          style = "aer",
          type = "html",
          title = "Table 2: 2021 EPA by Run Gap")
```
...
success_rate_runs %>%
  ...
  labs(x = "Success Rate on Runs",
       y = "Average Win Probability at All Times",
       title = "Graph 5: Rushing Success Rate") +
  #uses the black and white ggplot theme
  theme_bw() +
  #center title with hjust = 0.5
  theme(
...
runners %>%
  ...
  labs(x = "YPC",
       y = "EPA per play",
       title = "Graph 6: Rushing Efficiency 2021",
...
stargazer(EPA_Leaders_Twenty_One,
          ...
          style = "aer",
          type = "html",
          title = "Table 3: Passing EPA Leaders")
...
Non_QB_Passers %>%
  ...
  labs(x = "Yards",
       y = "EPA per Play",
       title = "Graph 7: Non-QB Passers EPA",
